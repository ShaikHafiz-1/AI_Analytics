...
def generate_forecast_answer(detail_records: list, context: dict, question: str) -> dict:
    """Generate answer for forecast quantity change questions"""
    from copilot_helpers import compute_forecast_metrics
    
    qty_records = [r for r in detail_records if r.get("qtyChanged", False)]
    if not qty_records:
        return {
            "answer": "No forecast quantity changes detected.",
            "supportingMetrics": {"qtyChangedCount": 0, "totalRecords": len(detail_records)}
        }
    
    metrics = compute_forecast_metrics(qty_records)
    
    answer = f"{metrics['qtyChangedCount']} records have forecast quantity changes. "
    answer += f"Total delta: {metrics['totalQtyDelta']:+,} units. "
    answer += f"Average delta: {metrics['averageQtyDelta']:+,.1f} units."
...
def generate_material_answer(detail_records: list, context: dict, question: str) -> dict:
    """Generate answer for material-specific questions"""
