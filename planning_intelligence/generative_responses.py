...
Supports both LLM-based and template-based generation with automatic fallback.
"""
...
class GenerativeResponseBuilder:
    """Build natural language responses from metrics using LLM or templates."""
    
    def __init__(self, use_llm: bool = True, detail_records: List[Dict] = None):
        """
        Initialize response builder.
        
        Args:
            use_llm: If True, use LLM for responses. If False, use templates only.
            detail_records: Optional list of actual blob records for LLM context
        """
        ...
        if use_llm:
            try:
                ...
                self.llm_service = get_llm_service(use_mock=True)  # Start with mock for testing
                logger.info(f"LLM service initialized. Available: {self.llm_service.is_available()}")
            except ImportError:
                logger.warning("Could not import LLM service. Using templates only.")
                self.use_llm = False
    ...
    def build_health_response(self, metrics: Dict) -> str:
        ...
        if self.use_llm and self.llm_service:
            try:
                ...
                logger.warning(f"LLM failed for health response: {str(e)}. Using template.")
        
        # Fallback to template
        return self._build_health_response_template(metrics)
    
    def _build_health_response_template(self, metrics: Dict) -> str:
        """Build health status response using templates."""
    ...
    def build_location_response(self, location: str, metrics: Dict) -> str:
        ...
        if self.use_llm and self.llm_service:
            try:
                context = {**metrics, "location": location}
                return self.llm_service.generate_response(
                    f"What is the status at {location}?",
                    context,
                    self.detail_records
                )
            except Exception as e:
                logger.warning(f"LLM failed for location response: {str(e)}. Using template.")
        
        # Fallback to template
        return self._build_location_response_template(location, metrics)
    
    def _build_location_response_template(self, location: str, metrics: Dict) -> str:
        """Build location-specific response using templates."""
    ...
    def build_design_response(self, metrics: Dict) -> str:
        ...
        if self.use_llm and self.llm_service:
            try:
                ...
                logger.warning(f"LLM failed for design response: {str(e)}. Using template.")
        
        # Fallback to template
        return self._build_design_response_template(metrics)
    
    def _build_design_response_template(self, metrics: Dict) -> str:
        """Build design change response using templates."""
    ...
    def build_forecast_response(self, metrics: Dict) -> str:
        ...
        if self.use_llm and self.llm_service:
            try:
                ...
                logger.warning(f"LLM failed for forecast response: {str(e)}. Using template.")
        
        # Fallback to template
        return self._build_forecast_response_template(metrics)
    
    def _build_forecast_response_template(self, metrics: Dict) -> str:
        """Build forecast response using templates."""
    ...
    def build_risk_response(self, metrics: Dict) -> str:
        ...
        if self.use_llm and self.llm_service:
            try:
                ...
                logger.warning(f"LLM failed for risk response: {str(e)}. Using template.")
        
        # Fallback to template
        return self._build_risk_response_template(metrics)
    
    def _build_risk_response_template(self, metrics: Dict) -> str:
        """Build risk assessment response using templates."""
        ...
        if self.use_llm and self.llm_service:
            try:
                context = {
                    "location1": loc1,
                    "location2": loc2,
                    "location1_records": metrics1.get("totalRecords", 0),
                    "location1_changes": metrics1.get("changedRecords", 0),
                    "location2_records": metrics2.get("totalRecords", 0),
                    "location2_changes": metrics2.get("changedRecords", 0),
                }
                return self.llm_service.generate_response(
                ...
                logger.warning(f"LLM failed for comparison response: {str(e)}. Using template.")
        
        # Fallback to template
        return self._build_comparison_response_template(loc1, metrics1, loc2, metrics2)
    
    def _build_comparison_response_template(self, loc1: str, metrics1: Dict, loc2: str, metrics2: Dict) -> str:
        """Build comparison response using templates."""
    ...
    def build_impact_response(self, metrics: Dict) -> str:
        ...
        if self.use_llm and self.llm_service:
            try:
                ...
                logger.warning(f"LLM failed for impact response: {str(e)}. Using template.")
        
        # Fallback to template
        return self._build_impact_response_template(metrics)
    
    def _build_impact_response_template(self, metrics: Dict) -> str:
        """Build impact analysis response using templates."""
...
def build_contextual_response(
    ...
    """
    builder = GenerativeResponseBuilder(detail_records=detail_records)
